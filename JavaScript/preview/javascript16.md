...
- 반드시 해당을 사용해 빈 객체를 생성해야 하는 것은 아니다. 객체 리터럴을 사용하는 것이 더 간편하다.
...
함수는 객체이므로 프로퍼티와 메소드를 소유할 수 있다. 

함수 객체는 함수로서 동작하기 위해 일반 객체의 내부 슬롯과 내부 메소드 이외에 내부 슬록과 내부 메소드를 추가적으로 가지고 있다.

- 내부 메소드 [[Call]]을 갖는 함수 객체 : callable
  - 호출할 수 있는 객체, 함수를 말한다. (함수 객체는 반드시 callable)
- 내부 메소드 [[Construct]]를 갖는 함수 객체 : construct
  - 생성자 함수로서 호출할 수 있는 객체를 의미한다. 이는 new 연산자(또는 super 연산자)와 함께 호출하는 것을 의미한다
...
1. 인스턴스 생성과 this 바인딩 암묵적으로 객체 생성된다. (이 빈 객체가 바로 생성자 함수가 생성한 인스턴스이다.) 생성자 함수가 생성한 인스턴스의 프로토타입으로 생성자 함수의 prototype 프로퍼티가 가리키는 객체가 설정된다. 암묵적으로 생성된 빈 객체, 즉 인스턴스는  this에 바인딩된다. 생성자 함수 내부의 this가 생성자 함수가 생성할 인스턴스를 가리키는 이유가 바로 이것이다.
...
- 추상연산?
- 생성자함수와 일반함수의 차이는? 단지 매개변수에 인자를 받도록 선언된 것을 말하는가?
- 5) 생성자 함수의 인스턴스 생성 과정
