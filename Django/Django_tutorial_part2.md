...
데이터베이스의 각 필드는 Field 클래스의 인스턴스로서 표현된다. 각각의 Field 인스턴스 이름인 기계가 읽기 좋은 형식의 데이터베이스 필드 이름이다. 이 필드명을 Python 코드에서 사용할 수 있으며, 데이터베이스에서는 컬럼명으로 사용할 것이다.

`ForeinKey`를 사용한 관계설정을 통해 위 코드에서는 각각의 Choice가 하나의 Question에 관계된다는 것을 Django에게 알려준다.
...
> 2. python manage.py makemigrations를 통해 이 변경 사항에 대한 마이그레이션을 만든다.
